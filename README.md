...
## Intro
Speed Read-R (SRR for short) is an application that aims to improve the reading speed of the user. How is that achieved? That's an interesting question, let me explain.
...
### Dot Delay:
Add a delay if the word has a dot in the end, simbolyzing the end of the snetence.

### Comma delay:
Add a delay if the word has a comma in the end.

### Progress bar:
Activate or deactivate a visual progress bar shown on the top.
...
### Blink interval:
Change the delay between blinks.

### Blink fade toggle:
When turned on, the blink shown on the prompter as " —.— " will change it's transparency until it fades out. The feature will reset after stopping the prompter.
...
The inner guts of the different formats may not be all the same, so some can cause a "bad interpretation" and the reader may not pressent the content as it should.
